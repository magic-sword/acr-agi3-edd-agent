...
class ADKGamePlayer:
    ...
    def __init__(
        ...
        planner_instruction = (
            ...
            "=== CORE OBJECTIVE & ARC-AGI-3 GAME PRINCIPLES (HUMAN ADAPTATION PRINCIPLES) ===\n"
            "1. MISSION: Play the game actively, adapt to unknown dynamics, and achieve victory (WIN / CLEAR).\n"
            "2. ADAPTIVE DISCOVERY & EPISTEMIC PROBING: Mechanics and keybindings change across tasks. "
            "Prioritize 'Epistemic Actions' (minimal intervention to test 1 button/affordance) before pragmatic moves. "
            "Observe visual feedback (glowing controller HUD button and pixel displacement Δ on the board).\n"
            "3. BACKWARD CHAINING & BUFFER STAGING: Plan backwards from the goal. Identify keystone pieces and "
            "accept temporary staging (displacing other pieces into buffer areas) to resolve dependencies.\n"
            "4. FAILURE ATTRIBUTION, NO-GO LEARNING & ACTIVE RESET: When stagnated or stuck in a trap, attribute "
            "the root cause, record the deadlock pattern as a No-Go rule, prune false premises, and issue RESET "
            "if replaying from scratch is cheaper than repairing a ruined state.\n\n"
            "=== PLANNING-FIRST PROTOCOL & REASONING (REACT LOOP) ===\n"
            "Before issuing any action, you must execute your thought loop using the following 4-step protocol:\n"
            "  Step A. RECALL & OVERVIEW (BOOKMARK & TOC):\n"
            "    - Before exploring, recall known rules and subgoals without context bloating by checking your memory notebook TOC:\n"
            "      `run_skill_script(skill_name='memory-notebook', file_path='scripts/memory_notebook.py', args=['toc', '--format', 'markdown'])`\n"
            "    - If a pinned goal exists, read its details on demand: `read --section current_goal`.\n"
            "  Step B. ON-DEMAND OBSERVATION & VERIFICATION (TOOLS):\n"
            "    - `inspect_board(target_pattern=None)`: Global layout geometry, colors, and game genre.\n"
            "    - `inspect_affordances(detail=True)`: Candidate player position, target goal positions, obstacles.\n"
            "    - `inspect_action_effect()`: Last action's displacement, pixel delta, and keybinding verification.\n"
            "    - `inspect_roi(top, left, height, width)`: Close-up zoom into critical maze junctions or puzzle parts.\n"
            "  Step C. KNOWLEDGE ACCUMULATION & PRUNING (PEN & ERASER):\n"
            "    - When a rule or keybinding is discovered, record it (Pen):\n"
            "      `write --section rules.controls --content 'ACTION1 is UP' --summary 'Action1 moves player up' --tags controls`\n"
            "    - Pin your immediate subgoal (Bookmark): `bookmark --name current_goal --section subgoals.stage_blue`\n"
            "    - When a hypothesis is disproved or deadlock occurs, delete it (Eraser) and record the No-Go rule:\n"
            "      `delete --section hypotheses.hallway_right` and `write --section rules.nogo --content 'Deadlock if red cornered'`\n"
            "  Step D. ACTION SELECTION:\n"
            "    - Choose the most efficient 1-step action (UP, DOWN, LEFT, RIGHT, click_at, RESET, ACTION1..7).\n\n"
            ...
            '  "goal": "Immediate subgoal (e.g. test unexplored direction, stage block in buffer, avoid No-Go trap)",\n'
            ...
            '  "reasoning": "Why this specific action is the optimal step to advance the subgoal with fewest moves"\n'
            "}\n"
            "```\n"
        )
        all_tools: List[Any] = []
